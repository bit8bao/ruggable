...
export const useFilters = (props: any) => {
  ...
  const setFilter = (id: any, val: any) => {
  ...
  const filteredRows = useMemo(() => {
    ...
    const filterRows = (rows: any) => {
      ...
      filteredRows = Object.entries(filters).reduce((filteredSoFar, [columnID, filterValue]) => {
        ...
        const column = columns.find((d: any) => d.id === columnID);
        ...
        return filteredSoFar.filter((row: any) => filterMethod(row, columnID, filterValue, column));
      }, rows);
